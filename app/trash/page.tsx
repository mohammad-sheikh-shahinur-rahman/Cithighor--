...
import { Search, Filter, Trash2, Star, Archive as ArchiveIcon, SortAsc, SortDesc, Tag, User, Calendar, Clock, RotateCcw } from "lucide-react"
...
export default function Trash() {
  ...
  useEffect(() => {
    // Load trash messages from localStorage
    const loadMessages = () => {
      const storedMessages = localStorage.getItem("trashMessages")
      if (storedMessages) {
  ...
  const handleRestore = (messageId: string) => {
    ...
    localStorage.setItem("trashMessages", JSON.stringify(updatedMessages))
    
    // Add back to sent messages
    const sentMessages = JSON.parse(localStorage.getItem("sentMessages") || "[]")
    const messageToRestore = messages.find(msg => msg.id === messageId)
    if (messageToRestore) {
      localStorage.setItem("sentMessages", JSON.stringify([...sentMessages, messageToRestore]))
    }
    
    toast({
      title: "সফল",
      description: "চিঠি পুনরুদ্ধার করা হয়েছে।",
    })
  }
  ...
  const handleDelete = (messageId: string) => {
    ...
    toast({
      title: "সফল",
      description: "চিঠি স্থায়ীভাবে মুছে ফেলা হয়েছে।",
    })
  }

  const handleEmptyTrash = () => {
    setMessages([])
    localStorage.setItem("trashMessages", JSON.stringify([]))
    
    toast({
      title: "সফল",
      description: "ট্র্যাশ খালি করা হয়েছে।",
    })
  }
      ...
      <main className="flex-1 container mx-auto p-4 md:p-6">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <h1 className="text-2xl font-serif font-bold text-amber-800">ট্র্যাশ</h1>
            ...
            <div className="flex items-center gap-2">
              ...
              {messages.length > 0 && (
                <Button
                  ...
                  className="text-red-50 hover:text-red-50 hover:bg-red-600"
                >
              ...
              <Card className="border-amber-200">
                <CardContent className="p-6">
                  {loading ? (
                    ...
                    <div className="text-center py-8 text-amber-700">ট্র্যাশে কোন চিঠি নেই।</div>
                  ) : (
